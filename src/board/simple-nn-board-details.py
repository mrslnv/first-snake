...
#trying to add detailed insights
...
#after 300 iterations, it is perfect

optimizer = tf.train.GradientDescentOptimizer(0.00017)

# optimizer = tf.train.GradientDescentOptimizer(0.00167)
# this is too much, it oscillates - it reaches perfect accuracy with delay
...
# ! How to run the next with different inupt without duplicating the chart
# * maybe, run session 2 times
# * how to compute/not computed summary node for accuracy?
logits_argmax = tf.argmax(logits3, 1, name="logits_argmax")
correct_prediction = tf.equal(tf.to_int32(logits_argmax), all_y)
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
acc_summary = tf.summary.scalar("accuracy",accuracy)
testWriter = tf.summary.FileWriter('summary-log')

for i in range(5000):
    sess.run(train, feed_dict={x: sample_x, y_: sample_y})
    if (i%100 == 0):
        sumOut = sess.run(summaryMerged, feed_dict={x: sample_x, y_: sample_y})
        writer.add_summary(sumOut,i)
        acc, acc_sum_out = sess.run([accuracy,acc_summary], feed_dict={x: all_x, y_: all_y})
        print("accuracy:",acc)
        testWriter.add_summary(acc_sum_out,i)
