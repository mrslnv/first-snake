...
W = tf.get_variable("W",shape=[2,4],initializer=initializer)
b = tf.get_variable("b",shape=[4],initializer=initializer)

# W = tf.Variable(initializer, dtype=tf.float32, expected_shape=[2, 2])
W1 = tf.get_variable("W1",shape=[4,4],initializer=initializer)
b1 = tf.get_variable("b1",shape=[4],initializer=initializer)

# W = tf.Variable(initializer, dtype=tf.float32, expected_shape=[2, 2])
W2 = tf.get_variable("W2",shape=[4,2],initializer=initializer)
...
logits2 = tf.matmul(relu2, W2) + b2

y_ = tf.placeholder(tf.int32, [None],name="y_")

loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits2,labels=y_,name="loss")
