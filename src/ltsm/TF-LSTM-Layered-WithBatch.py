...
# ToDo: difference: dynamic_rnn vs. static_rnn???!
...
state_per_layer_list_hidden = tf.unstack(init_state_hidden, axis=0)
state_per_layer_list_output = tf.unstack(init_state_output, axis=0)

rnn_tuple_state = tuple(
    [tf.nn.rnn_cell.LSTMStateTuple(state_per_layer_list_hidden[idx], state_per_layer_list_output[idx])
...
newcell = lambda: tf.nn.rnn_cell.BasicLSTMCell(state_size, state_is_tuple=True)
...
states_series, current_state = tf.nn.static_rnn(cell=cell, inputs=inputs_series, initial_state=rnn_tuple_state)
        ...
        for batch_idx in range(num_batches):
            ...
            _total_loss, _train_step, _current_state, _predictions_series = sess.run(
                ...
                feed_dict={
                    ...
                    init_state_hidden: _current_state_hidden,
                    init_state_output: _current_state_output
                })
